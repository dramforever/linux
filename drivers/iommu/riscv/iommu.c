...
static u64 riscv_iommu_pdtp_alloc(struct riscv_iommu_device *iommu)
{
	...
	pde = 0;
	for (i = 0; i < levels; i++) {
		pd[i] = iommu_alloc_pages_node_sz(dev_to_node(iommu->dev),
						  GFP_KERNEL_ACCOUNT, SZ_4K);
		if (!pd[i]) {
			/* allocation failed: do not enable pasid mode */
			while (i-- > 0)
				iommu_free_pages(pd[i]);
			return 0;
		}
		pd[i][0] = pde;
...
{
	/* Ignore errors here. If device doesn't support PRI, let it be. */
	riscv_iommu_dev_enable_iopf(dev);
}
...
static void riscv_iommu_release_device(struct device *dev)
{
	...
	riscv_iommu_dev_disable_iopf(dev);
